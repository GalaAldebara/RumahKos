        ...
        <div class="card shadow-sm p-4">
            ...
            <div class="d-flex justify-content-between align-items-start mb-4">
                <div>
                    ...
                    <p>{{ $data[0]->getuser->nama ?? 'Tidak ada data' }}</p>

                    <h5 class="fw-semibold mt-3">Tanggal Mulai:</h5>
                    <p>{{ date('d F Y', strtotime($data[0]->tanggal_pemesanan ?? now())) }}</p>

                    <h5 class="fw-semibold mt-3">Tanggal Selesai:</h5>
                    <p>{{ date('d F Y', strtotime($data[0]->dibooking_sampai ?? now())) }}</p>

                    <!-- Badge Status dan Tombol -->
                    <div class="mt-3 d-flex align-items-center gap-2">
                        @if ($data[0]->dibooking_sampai && $data[0]->dibooking_sampai > now())
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('perpanjangan_kontrak') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label for="durasi" class="form-label">Durasi Perpanjangan (bulan)</label>
                        <input type="number" class="form-control" id="durasi" name="durasi" placeholder="Masukkan durasi dalam bulan" required>
                    </div>
